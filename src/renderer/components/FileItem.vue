...
<template>
  <li class="item" :style="{ '--width': `${progress * 100}%` }">
    ...
    <div class="item__compress-ratio">
      {{ surplus ? `-${(((size - surplus) / size) * 100).toFixed(2)}%` : "-" }}
    </div>
    <button
      :disabled="progress !== 1"
      class="item__copy iconfont icon-copy"
      title="复制图片"
      @click="$emit('copy')"
    ></button>
    <button
      :disabled="progress !== 1"
      class="item__replace iconfont icon-icon_common_replace"
      title="覆盖原图"
      @click="$emit('replace')"
    ></button>
    <p class="item__tag">
